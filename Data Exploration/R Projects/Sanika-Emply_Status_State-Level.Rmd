...
```{r}
stateavg= mean(all_emp_stat$percent_in_labor_force_employed_disabled)
all_emp_stat$compared_to_state_avg_ed = ifelse(all_emp_stat$percent_in_labor_force_employed_disabled > stateavg , "above average",
                  ifelse(all_emp_stat$percent_in_labor_force_employed_disabled < stateavg, "below average",
                         "average"))
```


```{r}
colnames(all_emp_stat)[2] <-"States"
...
write.csv(all_emp_stat, "S:/misc/clg/DSPG2022/ProjectA/Sanika-Labor_Force_gaps_State-Level.csv", row.names = FALSE)
```
