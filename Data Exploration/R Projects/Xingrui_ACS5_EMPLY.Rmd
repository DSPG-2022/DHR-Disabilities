...
1.6 Testing the reliability of DiabledPop data
```{r}
CV_Total <- ((DisabledPop$moe_Total / 1.645) / DisabledPop$estimate_Total) * 100

DisabledPop$CV_Total <- ifelse(CV_Total > 30, "Low reliability",
...
DisabledVision <- c("18 to 34 years" = c("B18103_010", "B18103_029"), "35 to 64 years" = c("B18103_013", "B18103_032"))
...
DisabledIndependentLiving <- c("18 to 34 years" = c("B18107_004", "B18107_017"), "35 to 64 years" = c("B18107_007", "B18107_020"))
...
```
3.6 Difficulty type for all ages
```{r}
...
DisabledType_pivot <- DisabledType %>% separate(variable, into = c("Type", "Age"), sep = "\\.") %>%
  pivot_wider(names_from = Age, values_from = c(estimate, moe)) %>%
  ungroup() %>%
  mutate(Total = DisabledType_sum$estimate) %>%
  mutate(Percentage = rowSums(DisabledType_pivot[3:8], na.rm = TRUE) / DisabledType_sum$estimate * 100)

names(DisabledType_pivot) <- gsub("estimate_", "", names(DisabledType_pivot))
DisabledType_pivot$NAME <- gsub(" County, Iowa", "", DisabledType_pivot$NAME)
names(DisabledType_pivot)[names(DisabledType_pivot) == "NAME"] <- "County"

# write.xlsx(DisabledType_pivot, "Difficulty Type for all ages.xlsx")
```
...
7.1 IVRS hourly wage information and ACS median earnings
```{r}
...
8.1 Poverty level
```{r}
DisabledPop <- read.xlsx("DisabledPop.xlsx") %>%
  select(c(1, 6))

BelowPoverty <- get_acs(geography = "county",
                        variables = "B23024_003",
                        year = 2020,
                        state = "IA")
...
BelowPoverty <- BelowPoverty %>%
  cbind(DisabledPop) 
  # select(-c(2, 3)) %>%
  # mutate("Below Poverty" = estimate / Total.Disabled * 100)

write.xlsx(BelowPoverty, "Income in the past 12 months below poverty level.xlsx")
```