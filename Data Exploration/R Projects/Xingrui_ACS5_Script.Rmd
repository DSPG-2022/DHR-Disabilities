...
People with disabilities in Iowa are more common at ages 35 and older than at younger ages. One guess is that more people's disabilities are acquired.  
```{r}
...
Among all disabled people, American Indian and Alaska Native alone account for the largest proportion (16.8%), and Asian alone account for the least (5.7%).  
```{r}
...
(Wrong datasets!!!)Disability types did not show significant differences in gender. Hearing and visioning difficulties are difficulties for most disabled persons. 
```{r}
...
Difficulty$variable <- str_replace(Difficulty$variable, c("Male.", "Female."), "")

ggplot(Difficulty, aes(x = variable, y = estimate, fill = Sex)) +
  geom_bar(position="stack", stat="identity") +
  labs(title = "Disabled Population by Type of Disability",
       subtitle = "2020 American Community Survey",
       x = "Types of Disability",
       y = "Population") +
  scale_y_continuous(labels = scales::comma) +
  scale_fill_viridis_d()
```

The majority of disabled persons in the labor force are employed.  
```{r}
...
The employment rate of disabled persons in most counties is 80% and above, with a few outlier counties.  
```{r, results = "hide"}
...
People with disabilities tend to earn about 30% less than the median earnings of non-disabled people, and women in the disabled group tend to earn about 50% less than the median earnings of men.  
```{r}
...
A ratio of income to poverty below 1 is considered to be in poverty. The proportion of people with disabilities living in poverty varies from 10% to 30% in each Iowa county.  
```{r}
